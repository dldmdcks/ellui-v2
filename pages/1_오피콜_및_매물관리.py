...
all_records.reverse()
live_records.sort(key=lambda x: str(x[6])) 
        ...
        with st.form("new_data_form", clear_on_submit=True):
            ...
            if st.form_submit_button("🚀 매물방에 등록하기 (+3 토큰)", type="primary"):
                if not n_dong or not n_bon or not n_room: st.error("필수 항목(동, 번지, 호수)을 입력하세요.")
                    ...
                    # 🚨 에러 해결! [row_idx]용 빈값과 [d_day] 7을 추가하여 상자 30개로 맞춤
                    temp_live_row = new_row.copy()
                    temp_live_row.append(0) # row_idx 임시
                    temp_live_row.append(7) # d_day
