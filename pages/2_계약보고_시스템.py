...
if selected_tab == "✅ 계약 컨펌 요청":
    ...
    # 💡 [핵심] 다가구 동적 폼을 위해 '매물 종류'를 폼 바깥으로 뺌!
    c_btype1, c_btype2 = st.columns([1, 2])
    b_type = c_btype1.selectbox("📌 매물 용도 선택 (용도에 따라 양식이 변경됩니다)", ["오피스텔", "주택", "상가", "아파트", "다세대", "다가구", "빌라", "미분류"])

    with st.form("confirm_request_form", clear_on_submit=False):
        st.subheader("🏠 매물 및 계약 정보")
        ...
        landlord_name = c_p1.text_input("임대인 성함", placeholder="홍길동")
        tenant_name = c_p2.text_input("임차인 성함", placeholder="김철수")
        ...
        if submit_btn:
            ...
            elif not bunji or not landlord_name or not tenant_name: st.error("🚨 필수 항목(번지, 임대인, 임차인)을 입력해주세요.")
            elif b_type == "다가구" and (not senior_deposit.isdigit() or not total_rooms.isdigit()): st.error("🚨 다가구 주택은 선순위 총 보증금과 총 호실 수를 숫자로 정확히 입력해야 합니다.")
            else:
                addr_full = f"{cr_sido} {cr_gu} {dong} {bunji}{'-'+sub_dong if sub_dong else ''} {room}호"
                
                # 워크 메시지 포맷
                msg = f"[🔴 계약 컨펌 요청]\n"
                msg += f"담당자 : {user_name}\n"
                msg += f"주소 : {addr_full} [{b_type}]\n"
                msg += f"보/월 : {int(deposit):,}원 / {int(rent) if rent.isdigit() else 0:,}원\n"
                msg += f"임대인/임차인 : {landlord_name} / {tenant_name}\n"
