...
try: off_count = int(settings_all_values[6][1]) if len(settings_all_values)>6 else 4
except: off_count = 4
try: apt_count = int(settings_all_values[7][1]) if len(settings_all_values)>7 else 1
except: apt_count = 1
...
with c_set1:
    ...
    st.subheader("🎯 오피스텔 타겟 (주소 쉼표 구분)")
    with st.form("target_op_form", clear_on_submit=False):
        new_target_op = st.text_area("오피스텔/빌라 등 타겟 주소", value=target_op, height=120)
        if st.form_submit_button("💾 저장"):
...
with c_set2:
    ...
    st.subheader("🎯 아파트 타겟 (주소 쉼표 구분)")
    with st.form("target_apt_form", clear_on_submit=False):
        new_target_apt = st.text_area("아파트 타겟 주소", value=target_apt, height=120, placeholder="예: 신천동 17-6")
        if st.form_submit_button("💾 저장"):
